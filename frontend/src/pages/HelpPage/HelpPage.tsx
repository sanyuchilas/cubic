...
const HelpPage = () => {
    ...
    if (k === 1) {
      ...
      if (evt.target.value) {
        ...
        if (j === 1) {
          ...
          setProtocolValue(`Вопрос неверный`)
        }
      } else {
        setProtocolValue(``)
      }
  ...
  return (
    <div className={styles.container}>
      ...
      <p>
        Объект C-16-O. Занимался разработкой ЧВП в компании TimeStamp, впоследствии был уволен и помещен во временную систему T-16-G, состояющую из совокупности двух небольших ЧВП, созданных другим сотрудником по приказу менеджера компании, с <b>десятиминутным</b> жизненным циклом. C-16-O связался с вами и дал доступ к управлению T-16-G через внутренний компьютер. Пытался сбежать самостоятельно, но ничего не вышло...
      </p>
      ...
      {showSecret ?
        <>
          ...
          <p>
            Любая временная система может жить в двух направлениях: прямом и обратном. Любое такое направаление называется временным потоком. У ЧВП их всего два, в то время как у СВП их бесконечное множество, что логично
          </p>
